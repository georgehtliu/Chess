...
bool Knight::valid_path(Move mv) {
    return ((mv.x_dist() == mv.FORWARD_UNIT && mv.y_dist() == 2 * mv.FORWARD_UNIT) ||
            (mv.x_dist() == 2 * mv.FORWARD_UNIT && mv.y_dist() == mv.FORWARD_UNIT));
}
...
std::vector<std::pair<int, int>> Knight::generate_paths(std::pair<int, int> pos) {
    ...
    // 8 moves
    if (this->in_board(x - 2, y - 1)) paths.emplace_back(x - 2, y - 1);
    if (this->in_board(x - 2, y + 1)) paths.emplace_back(x - 2, y + 1);
    if (this->in_board(x + 2, y - 1)) paths.emplace_back(x + 2, y - 1);
    if (this->in_board(x + 2, y + 1)) paths.emplace_back(x + 2, y + 1);
    if (this->in_board(x - 1, y - 2)) paths.emplace_back(x - 1, y - 2);
    if (this->in_board(x - 1, y + 2)) paths.emplace_back(x - 1, y + 2);
    if (this->in_board(x + 1, y - 2)) paths.emplace_back(x + 1, y - 2);
    if (this->in_board(x + 1, y + 2)) paths.emplace_back(x + 1, y + 2);
